#!/usr/bin/env python3
"""3-features settings. Company-agnostic — set COMPANY per company; every path derives from one anchor.

The engine is the runnable twin of features.workflow.md: pick the top editorial blogs (script, via the
...
ROOT = os.path.dirname(HERE)                                  # .../3-features
...
WORK = os.path.join(BRAND_CTX, "_work", "features")
DRAFT_MD = os.path.join(BRAND_CTX, "features.md")   # THE deliverable — written in place;
...
RECIPE_MD = os.path.join(ROOT, "features.workflow.md")          # the template lives here (lifted verbatim)

PER_KIND_CAP = int(os.environ.get("FT_KIND_CAP", "25"))   # product/competitor/integration pages per kind, by traffic (pricing/compare/homepage always all)
WORKERS = int(os.environ.get("FT_WORKERS", "4"))
BODY_CHAR_CAP = int(os.environ.get("FT_BODY_CAP", "12000"))         # per blog inside a batch prompt
