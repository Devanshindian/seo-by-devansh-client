#!/usr/bin/env python3
"""4-writing-examples settings. Company-agnostic — set COMPANY per company; every path derives from one anchor.

The engine is the runnable twin of writing-examples.workflow.md: pick the top editorial blogs (script, via the
...
ROOT = os.path.dirname(HERE)                                  # .../4-writing-examples
...
WORK = os.path.join(BRAND_CTX, "_work", "writing-examples")
DRAFT_MD = os.path.join(BRAND_CTX, "writing-examples.md")   # THE deliverable — written in place;
...
RECIPE_MD = os.path.join(ROOT, "writing-examples.workflow.md")          # the template lives here (lifted verbatim)

KEEP = 5                                                    # the recipe's final count
BATCH = int(os.environ.get("WE_BATCH", "8"))              # articles scored per round (recipe: pick >=7)
MAX_ROUNDS = int(os.environ.get("WE_ROUNDS", "4"))        # scoring rounds before giving up loudly
WORKERS = int(os.environ.get("WE_WORKERS", "4"))
BODY_CHAR_CAP = int(os.environ.get("WE_BODY_CAP", "16000"))         # per blog inside a batch prompt
