#!/usr/bin/env python3
"""5-persona settings. Company-agnostic — set COMPANY per company; every path derives from one anchor.

The engine is the runnable twin of persona.workflow.md: pick the top editorial blogs (script, via the
...
ROOT = os.path.dirname(HERE)                                  # .../5-persona
...
WORK = os.path.join(BRAND_CTX, "_work", "persona")
DRAFT_MD = os.path.join(BRAND_CTX, "persona.md")   # THE deliverable — written in place;
...
RECIPE_MD = os.path.join(ROOT, "persona.workflow.md")          # the template lives here (lifted verbatim)
