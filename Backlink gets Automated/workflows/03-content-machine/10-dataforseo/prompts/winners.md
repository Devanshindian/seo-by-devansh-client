...
CONTEXT
- Distinct angle (what THIS asset covers, to spot the gaps): {{DISTINCT_ANGLE}}
...
- Note where any winner DRIFTS off the asset's topic (a pivot into an adjacent audience/topic), if any.
- Name the Gaps we can own — sub-topics thin or missing across winners, judged against the distinct angle
  (this is where the asset's angle beats the incumbents).
...
**Raw:** `<run_dir>/proof/05-pages.json` · read-list: `<run_dir>/proof/04-pages-to-read.txt`
