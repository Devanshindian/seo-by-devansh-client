...
The **keyword + SERP + AEO** engine of the research phase. Given one topic (Asset title + distinct angle), it
...
Seeds → expand the keyword net → filter by volume/KD → metrics → SERP + AI Overview → read top pages → AEO
...
├── prompts/                seeds · judge-keywords · score-keywords · serp-snapshot · winners · aeo-query · aeo-writeup · assemble · notes
└── scripts/
    ...
    ├── s6_aeo.py · s6_flow.py         Step 6 — AEO / LLM-mentions
