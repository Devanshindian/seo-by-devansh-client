...
class WikiWriter(dspy.Module):
    ...
    def forward(
        ...
        # BREADTH FLOOR: require at least 3 questions before the persona is allowed to end.
        # Rich (deep-RAG) answers make the persona satisfied after 1-2 turns, which starves
        # source breadth. This nudge forbids an early "Thank you" until 3 questions are asked.
        if len(dialogue_turns) < 3:
            conv += (
                f"\n\nYou have asked only {len(dialogue_turns)} question(s) so far. Do NOT end the "
                "conversation yet and do NOT say thank you. You must ask another substantive "
    ...
    def forward(self, topic: str, question: str, ground_truth_url: str):
        with dspy.settings.context(lm=self.engine, show_guidelines=False):
            ...
            if len(searched_results) > 0:
                # DEEP-RAG TWEAK: use ALL scraped passages per source (was r.snippets[:1] = first
                # chunk only), capped PER ARTICLE at 2500 words (was a shared 1000-word total).
                # All chunks from source n share citation index [n+1].
                info = ""
                for n, r in enumerate(searched_results):
                    src = "\n".join(f"[{n + 1}]: {s}" for s in r.snippets)
                    src = ArticleTextProcessing.limit_word_count_preserve_newline(src, 2500)
                    info += src + "\n\n"
                # global safety cap so one answer prompt can't run away (topk*2500 worst case)
                info = ArticleTextProcessing.limit_word_count_preserve_newline(info, 15000)
