...
def truncate_filename(filename, max_length=125):
    ...
    if len(filename) > max_length:
        ...
        # Testlify: was logging.warning — demoted to debug. We intentionally pass long topics (gap-check's full
        # gap-queries) and the run dir is renamed to iteration-N afterwards, so this truncation is harmless +
        # expected; the warning was just log noise.
        logging.debug(f"Filename too long; truncated to {truncated_filename}.")
...
class FileIOHelper:
    # LOCAL PATCH (revamp Phase 0.3b, [A#16]): writes go through an atomic temp-file+rename so a kill mid-write
    # (Ctrl-C, sleep, power, OOM) can never leave a half-written dossier that the gap-check gate ("file exists and
    # is non-empty") would then trust forever. The rename is atomic within one filesystem, so the target is only
    # ever the old-complete or new-complete version. Mirrors config.py C5 in our own engines.
    @staticmethod
    def _atomic_write(path, mode, encoding, writer):
        d = os.path.dirname(path) or "."
        fd, tmp = tempfile.mkstemp(dir=d, suffix=".tmp")
        try:
            with os.fdopen(fd, mode, encoding=encoding) if encoding else os.fdopen(fd, mode) as f:
                writer(f)
            os.replace(tmp, path)
    ...
    def dump_json(obj, file_name, encoding="utf-8"):
        FileIOHelper._atomic_write(
            ...
            lambda fw: json.dump(obj, fw, default=FileIOHelper.handle_non_serializable))
    ...
    def write_str(s, path):
        FileIOHelper._atomic_write(path, "w", None, lambda f: f.write(s))
