...
2. **STORM** — `run_storm.py "<primary kw>" --article --polish` in the STORM venv; conductor starts the shim first.
   *Output:* the polished dossier. *Gotcha:* the STORM folder name = `primary.replace(' ','_')` — must match what
   Steps 3-4 read (see `config.storm_dir`).
...
| 2 | `11-storm/run_storm.py` (+ `shim.py`, venv) |
