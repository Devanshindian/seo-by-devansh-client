...
Each run fires STORM: a research TEAM, not a lookup. Four researchers (a builder, a sceptic, an
evidence-checker and a practitioner) each interview an expert over several rounds, search the web behind
every question, and write up what they find. It is expensive, it takes hours, and it is built to explore a
SUBJECT AREA from several angles at once.
...
It is the WRONG tool for:
  ...
  Those are handled downstream — the architect researches what individual sections say they lack, and the
  write phase looks up a source for any specific number. Do NOT spend a STORM run on them.
...
1. JUDGE THE FLAGGED ITEMS. Decide which are worth filling. Drop a miss if it is minor, if it is really a
   rewording of another you are already covering, or if it is a single-fact hole (see above — not a STORM
   job). Not every gap deserves a run.
...
4. CLUSTER into AT MOST 3 research questions. Group related misses into one question — never one question
   per item. Each must be a broad, well-formed research topic a TEAM can build interviews around: it
   should be answerable from several angles, not by a single fact. If your question could be settled by
   one search, it is too narrow — either widen it into the area around it, or drop it.
...
Output STRICT JSON, nothing else:
{
  "queries": [
    {
      "query":  "<the research question for STORM — broad enough for a team, inside our world>",
