...
Steps:
  0 build checklist (PURE CODE — from 07-competitor-read.json + 04-serp-extract.json + spine.json) -> coverage-items.json
...
import config, checklist, judge, triage, rerun_storm, report
...
def main():
    ...
    print("== gap-check · Step 0: build checklist (code) ==")
    if a.redo or not have(items_p):
        data = checklist.build(a.brief, a.dossier)
        config.write_json(items_p, data)
        by = {}
        for it in data["items"]:
            by[it["type"]] = by.get(it["type"], 0) + 1
        print(f"  {len(data['items'])} items {by} -> {items_p}")
    else:
        print("  · cached")

    print("== gap-check · Step 1: coverage judge ==")
    if a.redo or not have(verdicts_p):
        judge.run(items_p, a.dossier, verdicts_p)
    else:
        print("  · cached")

    print("== gap-check · Step 2: gap triage ==")
    if a.redo or not have(queries_p):
        triage.run(verdicts_p, queries_p, a.dossier)
    else:
        print("  · cached")

    print("== gap-check · Step 3: STORM re-run ==")
    rerun_storm.run(queries_p, run_dir, a.dossier, redo=a.redo)

    print("== gap-check · Step 4: report ==")
    report.run(run_dir)
    print(f"\nDONE -> {run_dir}")
