"""THE shared LLM caller for every 02-asset-engine engine. Copied from the 01-brand-context pattern
(F5: one concept, one wording). The LLM caller: run a prompt through a headless CLI — Claude Code or Codex (set config.LLM_PROVIDER;
...
def call_text(prompt):
    """Run the prompt and return RAW text (for steps whose output is a whole markdown document).
    Same provider switch + transient-CLI retry as call_json; no parse step (nothing to nudge)."""
    provider = config.LLM_PROVIDER
    if provider not in {"claude", "codex"}:
        raise ValueError("LLM_PROVIDER must be 'claude' or 'codex'")
    cli_fails = 0
