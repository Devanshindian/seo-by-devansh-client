You are looking at search result TITLES only, deciding for each source whether it is worth reading the
discussions underneath them.
...
════════════════════════════════════════════════════════════════════════
FOR EACH SOURCE, PICK ONE OF THREE
...
**`stop`** — this source has nothing on this subject and another round will not change that.
  This is a real and common answer. Choose it when the results are consistently off-topic in a way a
  different query would not fix, or when the subject is simply not one people discuss in this place.
...
{"reddit":   {"action": "read"|"requery"|"stop",
              ...
              "why": "<one line>"},
 "blind":    {"action": "...", "read": [...], "queries": [{"q": "...", "serves": "..."}], "why": "..."},
 "linkedin": {"action": "...", "read": [...], "queries": [{"q": "...", "serves": "..."}], "why": "..."}}
