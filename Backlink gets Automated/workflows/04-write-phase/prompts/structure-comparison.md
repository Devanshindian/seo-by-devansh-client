...
ONE EXEMPTION, and only one: the OPTION sections. Every kept option above gets its own section even
when that puts the total over the ceiling — a comparison that silently omits one of the options it
was built to compare is worse than a long one. The ceiling therefore governs your SUPPORTING sections:
count the options first, subtract, and whatever is left is how many supporting sections you may add.
If that number is zero or less, add none.
...
2. Then the sections, in the exact order the final article should read. Give each option ONE section
   of its own; add supporting sections only where the material justifies them. For each one,
