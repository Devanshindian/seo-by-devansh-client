...
as_built_note: the write engine lives at workflows/04-write-phase/ (active build; planner+architect+writer ALL BUILT and run end to end on 5 articles as of 2026-08-01). For WHAT IS ACTUALLY BUILT and the latest decisions, read the "⭐⭐⭐ CURRENT BUILD STATE" block at the top of Part 4 (it supersedes everything below it, including the format-architect "not yet built" prose). THE BIG SPLIT (2026-07-26): the PLAN (Station 1) only VETS the material (select/rescue/cover/source-recover/source-cut) — it no longer merges/orders/shapes; the FORMAT ARCHITECT (write-phase step 0, design agreed, not yet built) shapes the pile to the format (SHAPE→MARSHAL→SELF-CHECK) and never fabricates. The format should also NUDGE the research phase (format brief → STORM) so structured articles arrive with structured inputs.
...
> **⭐ REVISED 2026-07-25 (as-built — see the AS-BUILT block at the top of Station 1):** `fmt_router.py` is built and
> is **mapping-only** (format name → archetype; unseen names get one LLM fallback). Two changes vs the table above:
> **(1)** the **Gate-0 class column is dropped** from the router — the research conductor already refuses non-writable
> ideas at pick time, so every bundle that reaches the write phase is writable; carrying the class map here was dead
> data. **(2)** `interactive-tool` and `case-study` archetypes are **removed from the write router** — tool ideas are
> filtered upstream (tool-build), and case-study is DATA-GATED (blocked pipeline-wide until customer stories are
> approved). The router therefore knows **7 archetypes + common-spine**. Add case-study's rows back the day stories
> are signed off. The other 7 rows of the table above are unchanged and correct.
...
---
 ...
 # ⭐⭐⭐ CURRENT BUILD STATE (updated 2026-08-01; what is ACTUALLY built + the latest decisions; supersedes everything below, including the 07-26 block)

> **The engine is a 3-runner pipeline** at `workflows/04-write-phase/` (23 scripts, ~30 prompts), output to
> `projects/<company>/04-write-phase/out/<slug>/{gather,planner,architect,writer}/` with `_work/` audit trails
> everywhere. All five pilot articles have run end to end: plan → structure → finished `draft.md`.
>
> **THE PLANNER** (`run_planner.py`): gather (carries the merit-picked `secondaries` keyword list; the old
> per-planned-H2 `h2_keywords` demoted to context) → select (H3 tagging with card receipts, 45% survival,
> orphan placement) → verify-sources → freeze → plan-view. **Source verification** is the deep step: AI picks
> which numeric cards need proof; the claimed page is fetched and judged (judge reads the page OPENING + the
> passages around the claim's numbers — a first-N-chars read showed only nav menus and false-cut ~21%);
> failures go to the HUNT: AI plans 3 queries/card → **ONE DataForSEO standard-queue batch** (post ≤100/req,
> wait once, collect via the REGULAR fetch — the advanced fetch silently doubles the price; measured
> $0.0006/search vs $0.002 live) → pages fetched (per-host lock + gap + retries) → AI judges, first hit wins →
> FIX the card's url, else CUT; proven-bad urls propagate to every citing card. Cap `WEB_MAX=250`.
> Claude-web fallback (serial) when credits are out. A failed search NEVER cuts a card.
>
> **THE ARCHITECT** (`run_architect.py`): shape (spine + per-section JOB before headline + flat box numbers +
> benched-with-reasons + `item_fields` per-item contract for listicles + needs_research markers) → enrich
> (markers → 3 queries → 15 pages → new cards ids 9001+, parallel 8 on DFS) → allocate (importance-based word
> shares, +20% buffer, code arithmetic) → **place-keywords (NEW 2026-08-01)**: maps the researched keyword set
> onto the article AS BUILT — confirms/re-picks the primary from the given pool only, writes the H1 (code pins
> the planned H1 whenever it already carries the primary), rewords headings where a keyword genuinely fits
> (ONE keyword per heading; never change a statement↔question shape; keyword-by-keyword matching against each
> section's JOB), and emits the ONE `keywords` block every later step reads. Guards in code: section count /
> order / jobs untouchable; redo always restarts from `_work/structure.pre-keywords.json` (write-once baseline).
>
> **THE WRITER** (`run_writer.py`): write-body (one AI per section, parallel; sees spine + full plan of jobs +
> own job/H3s/word target/cards + voice pack + persona; **keyword-blind by design**; FACT RULES vs WRITING
> RULES split; per-item contract injected + verified in code) → blend (the editor: 4 seam moves + weave the
> architect-confirmed keywords into prose — word swaps only; tag audit strips invented [c] ids, multi-id
> blocks `[c1, c2]` handled by the shared `tags.py`) → wrapper (H1 exactly as given; answer-first intro with
> the primary in the first 100 words, banned self-describing openers; 3-5 key takeaways; EXACTLY 5 FAQs via
> the 4-part test + no-two-share-an-answer; close = forward CTA grounded in features.md + primary where
> natural; touch-ups to the 2 new seams) → assemble (`draft.md` + `keyword-coverage.json` counted by CODE,
> hyphen-insensitive + `article.html`). **No TL;DR** (key takeaways covers it — deliberate deviation).
>
> **EVAL PAGES (all built, auto-rebuilt each run):** gather-review · selection-review · sources-review (every
> checked claim, filterable verdicts) · blueprint-review (spine, bench, keyword decision, word shares) ·
> article.html (hover sources, colour-coded keywords, coverage panel, edit trail with declared-vs-actual diffs).
>
> **MODELS:** default chain DeepSeek v4-pro → Claude → Codex; `llm.max_workers()` provider-aware. A/B (5
> articles, blind reviewers + judge): Claude Sonnet better prose/discipline, DeepSeek better structural
> compliance; most differentiators were PROMPT gaps, since fixed. Default NOT switched. Known: nothing records
> which model wrote an output (open). Costs measured: searches ~$0.002 live / $0.0006 queued; ~$0.50/article
> planner-run search spend pre-batching.
>
> **CHECKLIST COVERAGE (vs seo-aeo-geo-checklist):** built = keywords/H1/first-100/close/headings placement,
> 5-FAQ, sources integrity, brand-in-every-table rule (conditional, brand-agnostic), per-item contract.
> **NOT built:** CLEAN (station 3) · MEASURE facts.json (4) · REVIEW (5) · EDIT loop incl avoid-ai-writing (6)
> · GATE scoring (7) · PUBLISH (8, parked) · meta step · internal linker (needs live-URL list) · 40-60w
> liftable answers under question H2s · E-E-A-T author box/dates (publish-time; author identity = OPEN
> DECISION) · competitor-link check. Known counter artifacts to fix: primary-in-heading not counted, scare
> quotes break exact match, 2-3 upper bound too strict, heading keywords not counted as "used".
>
> **NEXT (Devansh's order, 2026-08-01):**
>   1. **AI-SLOP PASS first** (station 6b pulled forward): an editor fed the avoid-ai-writing ruleset strips
>      the tells (em-dash chains, Tier-1 words, tricolons, uniform rhythm, chatbot artifacts); may not touch
>      facts/numbers/headings/[c] tags (same tag audit as blend). The step counts its own targets before and
>      after (em-dashes, banned words) and prints the proof — full measurement comes later with facts.json.
>   2. **LINKS second:** external = from the article's cited sources, LINK only credible non-competitor ones
>      (code-filtered blocklist + AI pick); internal = fetch the company sitemap → AI matches 3-5 genuinely
>      relevant pages to existing sentences, varied anchors, + one product link where natural.
>      PENDING DEVANSH: published sources list curated-vs-full · competitor-blocklist approach · external
>      link count target.
>   3. **CLEAN** (pure code: invisible chars · whitespace · quote/dash normalisation) then **MEASURE** →
>      facts.json (words vs targets · sentence/paragraph stats · em-dash rate · Tier-1 words · citations +
>      per-domain counts · heading hierarchy/spacing/question-share · 40-60w liftable-answer detection).
>   4. Then ACT on measure: REVIEW + EDIT + GATE (one loop) → meta · liftable-answer rule · publish.
> DONE 2026-08-01: the 4 coverage-counter fixes (quote-tolerant matching; primary counts in the headings
> check; "2-3" → "≥2"; heading keywords count as used) + free recount — 3 articles all-green, video +
> listicle red marks now GENUINE research-phase ceilings (starved pool / question-format headings).
> Open with Devansh: author identity (E-E-A-T), model provenance stamping, card overload (up to 3,226 cards →
> 3,960w article), listicle primer placement, WP publish.

# ⭐ historical build state (2026-07-26; superseded by the block above)
>
> **Engine:** `workflows/04-write-phase/` (a top-level stage; the old `15-write` duplicate was deleted). Output:
> `projects/<company>/04-write-phase/out/<slug>/`. Config CLEAN (no SEO thresholds — owned by the checklist).
> Concurrency PROVIDER-AWARE (`llm.max_workers()`: DeepSeek parallel, Claude/Codex serial). `.env` via `config.REPO_ROOT`.
>
> ## THE BIG SPLIT (decided 2026-07-26): the PLAN vets the material; the WRITE phase SHAPES it to the format.
> The format archetype was OVERCLAIMED before ("Station 1 shapes the article to the format"). In truth it only ever did
> two soft things and touched the cards not at all: it **labelled** each section with a slot (one of 3 keep-signals) and
> **roughly ordered** them. It never created missing sections, deduped slots, decided parameters, or picked cards. So we
> split the responsibility cleanly:
>   - **Station 1 (PLAN) = VET the raw material.** Keep/cut sections, rescue gold, cover the gaps, source everything, cut
>     the unsourceable. Output = a vetted, UNORDERED, UNMERGED pile of sections+cards.
>   - **Write phase, step 0 = the FORMAT ARCHITECT.** It shapes that pile to the format (below).
> **REMOVED from Station 1:** the merge/order step (`plan-merge-order.md` + `_load_assembly`) — it moved into the architect.
> Keep/cut still uses the format's `## Slots` as a soft signal (that's fine); nothing else format-structural stays here.
>
> ## Station 1 (PLAN) — the pipeline as built (generalized `--slug`; orchestrated by `run_all.py`):
> 1. `fmt_router.route_slug` — format → archetype (table + LLM fallback). **News that is data-heavy → data-benchmark (one AI check).**
> 2. `gather_inputs.py` → `plan-inputs.json` — Group A + B. PAA in `group_b.paa_pool`; every card carries `source_urls`.
> 3. `plan_select.py` → `article-plan.draft.json` — **PICK ONLY.** Judge each H2 keep/cut + a loose slot + gaps + rescue
>    gold. A kept section keeps ALL its cards. **NO merge, NO order, NO shape** — it emits raw vetted material.
> 4. `place_rescued.py` — file rescued gold into the best kept section. **Never silently drops** — the fit-nowhere
>    remainder goes to a visible `rescued_unplaced` bench (assert: nothing is lost). Most place fine; the bench is small.
> 5. `coverage.py` → `coverage.json` — STRICT, evidence-based (G=gaps, T=must-cover, Q=PAA; related searches excluded).
>    **Re-run AFTER gap-fill** (`coverage2` in run_all) so the report is never stale.
> 6. `fill_gaps.py` — web-search sourced facts for a real hole (Claude CLI WebSearch, free) → place in the right section.
> 7. `source_gate.py` — **the source safety-net.** Any NUMERIC card still unsourced after recovery is REMOVED from the
>    plan and logged to `source-cuts.json` (the writer never gets an uncitable number). Tier-2 DataForSEO search is BUILT
>    but DORMANT (no credits) — it self-skips and just flags → cut. Non-numeric claims are left alone.
> 8. `freeze.py` → `article-plan.json` — lock + sanity-check, split HARD vs SOFT. **HARD** (no sections / a 0-card
>    section) → `run_all` REDOES the article (bounded 2×, then `NEEDS REVIEW`). **SOFT** (coverage holes, often
>    unfillable) → just noted, no redo.
> 9. `plan_view.py` → `article-plan.md` — the readable outline.
>
> ## Source recovery (a research-phase leak, fixed; feeds the write phase) — no web search, no fabrication:
> STORM writes some sentences without a `[n]` footnote → those cards lost their URL (100% of the unsourced cards are
> `storm`; `web`/`ownpage` are always sourced). The URL is still in STORM's own research store (`url_to_info.json`).
> `source_match.py` recovers it by matching the card's text to the snippet that CONTAINS the fact (number-normalized +
> a number-window path; ~80-95% recovery, VERIFIED — the number itself appears in the match). Applied at ORIGIN
> (`harvest_storm.py`, future articles) and IN PLACE to the 5 existing bundles (`reattach_sources.py`).
>
> ## THE FORMAT ARCHITECT (write phase, step 0) — the missing brain. "Turns the pantry into a recipe." (design agreed; NOT built yet)
> Reads: the vetted pile + the FULL format file (all 4 sections: Slots, Structure, Craft, Checks) + settings. Works on
> card ONE-LINERS at scale, attaches full text by id. Writes `article-blueprint.json`. Three steps:
>   - **A. SHAPE** → `blueprint.json`: the ordered target sections. CREATES missing required sections (a definitional's
>     40-60w answer block; a comparison's criteria block + table), MERGES duplicates, kills duplicate slots, ORDERS by
>     the format. For STRUCTURED formats (comparison / listicle / data-benchmark) it also identifies the ENTITIES (the
>     tools / items) and the YARDSTICKS (the common comparison dimensions / per-item fields / metrics). No cards yet.
>   - **B. MARSHAL** → `article-blueprint.json`: per section, pick the RIGHT cards (from the mapped raw sections + the
>     whole pool for created sections + the rescue bench). **This is the card-level selection Station 1 never did.**
>     No card cap (1..N per section/yardstick). Cards that fit nowhere are dropped — EXCEPT protected stats (PROTECT
>     rule) — and every drop is logged.
>   - **C. SELF-CHECK**: run the format's `## Format-specific checks` (answer-block genuinely first? table has every
>     column? every option covers every yardstick + names an honest weakness? own-domain not over-cited?). Fix or flag.
>     This is the guard that catches the "secretly a Testlify ad" comparison.
> **THE IRON RULE: the architect SHAPES, PROTECTS, and FLAGS — it NEVER fabricates.** A missing yardstick = `—`, not a
> guess. Not enough competitors = a FLAG, not an invention. A data-report methodology we don't have = describe the
> AGGREGATION honestly (which sources, which dates — the dossier now carries these) OR downgrade the claim; never fake
> original research. Then the Station-2 section-writers (dumb + parallel) fill each blueprint section.
>
> ## THE DEEPER REALIZATION: the format should NUDGE the RESEARCH phase too (a nudge, never a gate).
> Structured articles come out thin because research gathered "stuff about the topic," not "these competitors on these
> COMMON axes" / "the methodology." The format is already known at research time (`research-log.csv` `format` column,
> read by `14-research-conductor/run_research.py`). So the conductor should build a light **format brief** from the
> archetype and inject it into STORM's topic/persona (`11-storm/run_storm.py`) — a NUDGE ("this becomes a comparison —
> while researching broadly, ALSO surface the competing tools + comparable facts for each: pricing, 1-2 headline
> features, one proof metric") that ADDS to the broad research without forcing or breaking it. The architect still
> degrades gracefully (shape + flag) when inputs are thin; the nudge just makes them less thin. (Research-phase task,
> not yet built.)
>
> ## ⭐ 2026-07-26 pm — ARCHITECT additions + the STATION 2 (WRITE) design (decided with Devansh; architect BUILT, writer NOT)
>
> **Merge/order REMOVED from Station 1** — `plan_select` is PICK-only; it hands over raw vetted sections, and ALL
> shaping (merge/order/create/regroup) is the architect's. The old "only 4 sections" tiny plans were that merge step
> collapsing the ~16 kept sections — gone now. (Re-run Station 1 so the plans carry the full kept sections.)
>
> **Keywords: inherit or EMPTY — never invent.** A final section keeps the keyword(s) of the rough sections it's made
> from; a brand-new block (tool-block / table / wrapper) with no source carries NO keyword. We never generate or fall
> back to one. The real, research-validated keywords all survive on the topic sections.
>
> **Architect now builds the WRAPPER layer (was missing).** SHAPE wraps the body in the common house order:
> `intro → TL;DR → [body] → key-takeaways → CTA → FAQ(last)`. The body's own opener (answer-block / verdict-intro)
> stays first; an existing FAQ-like section (related-questions) BECOMES the FAQ. **FAQ is fed `paa_pool`** (MARSHAL
> assigns the cards that answer each PAA question); **CTA is fed `close_spec`** (confirmed high quality — no separate
> build). intro / TL;DR / key-takeaways carry the protected unique-angle highlight cards.
>
> **CHECK additions (inside the architect):** (a) FAQ answers ≥ min(5, #PAA) questions — an ACTION (we have the
> questions + the cards); (b) each `winners_common_h2s` table-stakes topic is covered somewhere — **FLAG ONLY, never
> force-fit** (Station 1 + gap-fill already tried; still-missing ⇒ we lack material or it's off-angle; a human decides).
> Plus the format's own checks + the uniqueness (angle-survived) guard. The final `article-blueprint.json` now also
> carries **`persona` + `word_band`** for the writer.
>
> **Inputs audit** — architect USES: format · cards · gaps_to_own · paa_pool · winners_common_h2s (flag) · close_spec ·
> persona · word_band. Deliberately UNUSED: keywords beyond inherited, winners_drift, related_searches (junk).
>
> ### STATION 2 — WRITE: refined build shape (BUILT 2026-07-27; supersedes the older Station 2 prose below)
> **AS BUILT (2026-07-27):** `run_write.py` orchestrates BODY (`write_body.py`) → WRAPPERS (`write_wrappers.py`:
> intro/tldr/key-takeaways/cta) → FAQ (`write_faq.py`) → PRODUCT-FIT (`product_fit.py`, ≤3 relevance-gated Testlify
> mentions from features.md) → META (`meta.py`) → ASSEMBLE (`assemble.py` → `draft.md`). Body is product-free; each
> fact carries a `[c<id>]` tag inline → code builds a per-claim provenance list → assemble turns each tag into a
> `([source](url))` link. INTERNAL-LINKS step deferred (the source-vs-internal-link overlap below is still open).
> Ran end-to-end on all 5 formats (first real drafts produced). `llm.py` now falls back DeepSeek v4-pro → Claude →
> Codex. KNOWN POLISH: the code-rendered comparison table has uneven cells; consecutive duplicate source-links.
> Body-FIRST and PRODUCT-FREE, then deliberate post-passes:
> 1. **Body (product-free)** — always-on inputs: `brand-voice` · `style-guide` · `writing-examples` · `writing-integrity`
>    · persona; plus the section's cards (full text + source_urls) + the format's craft. Cite each stat's source inline.
> 2. **FAQ** (from the blueprint's `faq_answers`, ≤40w each) + **wrappers** (intro / TL;DR / key-takeaways / CTA-from-close_spec).
> 3. **PRODUCT-FIT pass** — insert a Testlify mention ONLY where it genuinely earns its place (`features.md`,
>    relevance-gated). The body is written objective; product is added deliberately, never forced.
> 4. **INTERNAL-LINK pass** — a SELECTOR over the bundle's candidate internal links, placed into the finished prose.
> 5. **META** (title + description), then `assemble.py` → `draft.md`.
> 6. The **SEO/AEO/GEO checklist is the FINAL check** (Station 4/7), NOT a writing input.
>
> **OPEN QUESTION (parked, revisit):** internal links vs source links OVERLAP. A card's `source_urls` are fact
> CITATIONS — usually external, but `ownpage` cards cite testlify.com, so some sources ARE internal. The SEO
> internal-linker adds navigational links to RELATED Testlify pages (from the bundle), which may or may not be a card's
> source. Decide how the two relate: does an ownpage source double as an internal link? is source-citation (during
> writing) separate from SEO internal-linking (after)? Devansh flagged this; not yet resolved.
>
> ---
>
> # ⭐⭐ AS-BUILT DESIGN — Station 1 (recorded 2026-07-25, historical — see the CURRENT BUILD STATE above for what shipped)
>
> This block is the current, agreed design for Station 1, worked out field-by-field with Devansh. Where the older
> prose in the rest of Station 1 conflicts with this, **this wins.** It is detailed enough to rebuild from scratch.
> The engine lives at `workflows/03-content-machine/15-write/`.
>
> ## Already built
> - **Scaffold:** `config.py` (lean — ONLY paths the built code reads; company = one setting; NO SEO-rule constants,
>   those are owned by the checklist), `llm.py` (`call_text` for prose, `call_json` for judgment; claude default,
>   codex/deepseek switch), `write_atomic.py` (crash-safe temp-file→rename saves).
> - **`fmt_router.py`** — mapping-only, format name → one of **7 archetypes + common-spine** (interactive-tool +
>   case-study + the Gate-0 class map are removed — see the REVISED note under §3.1). Table lookup; unseen name → one
>   LLM fallback (`prompts/route-format-fallback.md`). Never keyword-matches the title.
>
> ## Station 1 runs in TWO sub-steps (this is the key structural decision)
> - **Step 1a — `gather_inputs.py` → `plan-inputs.json` (the "resource table").** DETERMINISTIC gather of the raw
>   material (Groups A + B below). No section decisions. The only judgment is ONE `call_json` LLM-extract for the
>   four prose Build-spec fields, with a label-parse fallback.
> - **Step 1b — `plan_select.py` → `article-plan.json` (Group C).** The AI-judgment step (Chunk 2). It LOADS THE
>   FORMAT SPEC FIRST (the archetype's skeleton + rules — the "format checklist"), then selects/decides. It cannot
>   run until the routed archetype's format spec exists (a human-reviewed artefact, built per format).
> - **`plan_assert.py`** — the Kind-3 code checks that freeze the plan (Chunk 3).
>
> ## The resource table — GROUP A (article settings) + GROUP B (selection raw material)
> Each field: source · always-there? · fallback · lifted/decided.
>
> **Group A — article settings (all ALWAYS present):**
> - `slug` — structure JSON — lifted.
> - `h1` — the asset title (structure JSON) — lifted. **H1 RULE (revised):** the asset title is creative and is
>   NOT keyword-front and NOT ≤60 chars — and that is FINE. Best practice: the ≤60-char + keyword-front rule is the
>   **meta title's** job (written at Station 2), NOT the H1's. Station 1 only ensures the primary keyword **or a
>   close variant** appears in the H1 (loose match); if wholly absent, lightly tweak to include a variant. No char cap on H1.
> - `format_archetype` — `fmt_router.py` — lifted.
> - `primary_keyword` + `keyword_set` (variations/secondaries/in_body/h2_keywords) — structure JSON (clean) — lifted.
> - `persona` {name,lens,why} — structure JSON (clean) — lifted.
> - `word_band` {min,max} — research-doc prose — **LLM-extract → label-parse fallback**; if only one number, `max =
>   min×1.2`; it is a REPORTED target, never a hard gate (length is an output).
> - `close_spec` — research-doc prose — LLM-extract → parse. (Verified good quality across bundles.)
>
> **Group B — selection raw material:**
> - `sections_menu` (~18-21 candidate H2s, each with H3s + evidence cards {card_id,gloss,verbatim,source_urls,tag}) —
>   structure JSON — ALWAYS — lifted. (Read evidence wherever it lives: H3 OR section.)
> - `gaps_to_own` — `proof/05-winners.md` (parse by the stable `**Gaps we can own:**` label) — ALWAYS (winners.md
>   prompt guarantees it) — lifted.
> - `winners_common_h2s` — `proof/05-winners.md` — ALWAYS — lifted. Table-stakes topics; used as a SANITY CHECK.
> - `confirmed_format` (winning shape + competitor depths) — `proof/05-winners.md` — ALWAYS — lifted. Informs word_band.
> - `winners_drift` (tangents to AVOID) — `proof/05-winners.md` — usually (can be empty) — lifted.
> - `paa_pool` (FAQ candidate questions) — `proof/04-serp-extract.json` (clean JSON) — **MAYBE** (was 0 for one topic)
>   — fallback: author the FAQ from the cards/topic — lifted.
> - `related_searches` (on-angle) — `proof/04-serp-extract.json` (clean JSON) — usually (16× in samples) — lifted.
>
> **How each field is produced (provenance, verified):** keyword_set = deterministic code (`keyword_set.py`) from
> DataForSEO; persona = `pick_persona.py` (AI, fixed JSON); word_band/snippet-suggestion/primary-source-suggestion/
> close = `s7_assemble.py` + `prompts/assemble.md` (AI synthesis, PROSE); winners blocks = `s5b_winners.py` +
> `prompts/winners.md` (AI over REAL scraped competitor pages); paa/related/featured_snippet = DataForSEO SERP pull.
> **AI Overview is present in the dossier but is NEVER used as content. The featured snippet was `null` (open) in
> every sampled topic — so we always AUTHOR our own, never reuse a competitor's or the AI Overview's.**
>
> ## GROUP C — decided at Station 1b (Chunk 2), IN THIS ORDER (each depends on the previous)
> 1. **`sections`** — keep/merge/cut the menu. A section survives only if it (a) covers a `winners_common_h2`
>    (table-stakes) OR closes a `gap_to_own` OR holds strong unique evidence, (b) is on-angle (not a `winners_drift`
>    tangent), (c) fits the loaded format skeleton. **PROTECT rule:** anything carrying a hard stat / a gap / the
>    differentiator is never cut. Count guided by format + word_band (definitional = 4-7 H2s). Dedupe same-keyword sections.
> 2. **evidence cards** assigned to the chosen sections/H3s — each `card_id` in exactly ONE section.
> 3. **`snippet_plan` {section, format}** — which chosen section wins the answer box + the format (para/list/table,
>    matched to the SERP's open format). TEXT is written at Station 2, NOT here.
> 4. **`primary_sources`** — the 2-4 most authoritative URLs **chosen from the selected cards' real `source_urls`**
>    (grounded, verifiable). The research-doc's LLM-suggested source list is DROPPED — no memory-invented sources.
> 5. **`faq_questions`** — authored from `paa_pool` (+ `related_searches`); if `paa_pool` is empty, author from the
>    cards/topic; drop any question the chosen body already answers.
> 6. **`standout_points`** (NEW) — the 2-4 most DISTINCTIVE facts/stats/framings mined from the FULL dossier (STORM
>    dossier + all cards, not just the menu) that competitors lack, each grounded in a real card. This is the
>    deliberate "use the dossier's uniqueness, don't just match the checklist" step. (Distinct from `gaps_to_own`:
>    gaps = topics competitors MISS; standout_points = distinctive facts we HAVE.)
> 7. **`coverage_map`** (NEW) — a traceability record: every `gap_to_own` → its section; every `winners_common_h2` →
>    its section OR an explicit "skipped, because…"; every `paa` → a FAQ/section. Station 1b fails loudly if a
>    table-stakes topic or a gap maps nowhere. Station 4 later verifies the DRAFT actually delivered each.
>
> ## Chunk 2 (`plan_select.py`) — the PER-H2, CARD-LEVEL selection design (2026-07-25; supersedes the batched heading-only version)
>
> **Goal:** for each candidate H2, decide keep/cut AND pick the exact cards that matter (by id, tagged to WHY). Then
> club, review, merge, and gap-fill into the final outline. Group A + B are the messy dossier's useful bits, tidied.
>
> **How each H2 is judged — ONE AI call PER H2 (~21 calls; precision over speed, on purpose):**
> - Give the call: the H2 + its H3 titles + ITS cards. Each card = `id · gloss (one-liner)`, PLUS its `verbatim`
>   ONLY for cards a code pre-scan flagged as carrying a real figure. (VERIFIED: glosses are topic-level — only ~17%
>   carry a number, but ~93% of verbatims do, and ~77% of cards have a number the gloss dropped. So topic-matching
>   uses glosses; the "carries data" test must read the verbatim. Code finds the figures, the AI judges if they matter.)
> - Hand it a CHECKLIST to match the H2 against: the format's SLOT list (which one slot does this H2 fill, or none) ·
>   `gaps_to_own` · `winners_common_h2s` · `winners_drift` (avoid) · "carries data?" (the flagged cards) ·
>   "uniquely valuable?" (rare/surprising/own-able).
> - Output per H2 (a small table): the verdict, the slot, and for each checklist item WHICH card_ids satisfy it, plus
>   `relevant_card_ids` = the union of every cited card (the cards worth keeping for this section).
> - **Keep/cut falls out of the match:** satisfies at least one of {a slot, a gap, a table-stakes topic, a data card,
>   a unique card} → keep; nothing → cut. PROTECT is built in (a data or unique or gap card = automatic keep).
>
> **The format splits two ways (the slots-vs-assembly decision):**
> - **SLOTS → the per-H2 prompt** (from each format file's `## Slots`): the named slots an H2 can fill. Per-H2 the AI
>   only labels "which one slot, or none".
> - **ASSEMBLY RULES → the combine step** (order of slots · count guidance · special flags: mode gate / A-vs-B /
>   hub-vs-entry · snippet_plan rule). These describe the WHOLE article, so they apply when the H2s are combined, never per-H2.
>
> **Length is NOT a selection constraint** — keep every H2 that earns its place; `word_band` is a soft guide, not a cap.
>
> **The pipeline (build Stage 1 first, then 2 and 3):**
> 1. **PICK (per-H2, card-level)** — the ~21 per-H2 calls above → each kept H2 with its `relevant_card_ids` + slot +
>    which gaps/topics/data/unique it covers. Then MERGE + ORDER (one call over the survivors, using the assembly rules).
> 2. **REVIEW & AUGMENT (whole-article)** — a pass over the full picked set + what was cut: did we miss an angle,
>    over-repeat one, or wrongly cut a chunk? Pull back what belongs. (Per-H2 judging is blind to the big picture — this is its backstop.)
> 3. **GAP-FILL** — for each checklist item still unmet across the whole article, WEB-SEARCH to mint new cards (new
>    ids) that fit the format + angle. Placed last, so we research only genuine holes.
>
> Master table after the loop: every final H2 with its cards (each card tagged to its owning H2 + the criterion it
> satisfies). Then produce the rest of Group C (snippet_plan · primary_sources from the kept cards' source_urls ·
> faq_questions from paa_pool · standout_points from the `unique_value` cards · coverage_map from the per-H2 matches),
> then Chunk 3 freezes it.
>
> ## Chunk 3 — `plan_assert.py` freeze checks
> - every chosen `card_id` in exactly one section · primary keyword mapped (H1 loose-match, first-100, 2-3 H2s,
>   conclusion; nullable target_keyword allowed on differentiator sections) · section count fits the loaded format
>   skeleton (not a fixed number) · ≥50% question H2s for question-led formats · `coverage_map` complete (every
>   common_h2 + gap mapped or justified-skip; every paa → faq/section) · `faq_questions` present · `snippet_plan` set
>   · `primary_sources` ≥2 and EVERY one drawn from a selected card's `source_urls` · `standout_points` present and
>   each grounded in a real card · wrapper slots reserved in the fixed house order (Intro → TL;DR → body → Key
>   takeaways → one CTA → FAQ last).
>
> ## The prose seams (recorded so we don't rediscover them)
> Four Build-spec fields (`word_band`, snippet suggestion, source suggestion, `close_spec`) and the winners blocks
> (`gaps_to_own`, `winners_common_h2s`, `confirmed_format`, `winners_drift`) live in PROSE (markdown), not JSON.
> Labels are consistent across all sampled bundles, so parse-by-label is reliable; word_band/close use LLM-extract
> with a parse fallback. `paa_pool`/`related_searches` are clean JSON. **Deferred upstream improvement:** have the
> DataForSEO engine ALSO emit a `build-spec.json` + structured gaps, so the handoff is fully structured.
>
> ## Build order for Station 1
> 1. `gather_inputs.py` (Group A+B → `plan-inputs.json`) — no format-spec dependency, build + verify first.
> 2. define the routed archetype's **format spec** (human-reviewed, per format) — prerequisite for 1b.
> 3. `plan_select.py` (Group C → `article-plan.json`) — Chunk 2.
> 4. `plan_assert.py` (freeze checks) — Chunk 3.
