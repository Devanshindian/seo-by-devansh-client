#!/usr/bin/env python3
"""The [c<id>] source tag — ONE definition, used by every step that reads or rewrites prose.
...
The RANGE form was the same bug a second time (2026-08-06): a writer emitted "[c221–c227]" with an
EN DASH, no step matched it, and it printed raw inside a published table. A range means every id from
first to last, so it is expanded — matching only the two endpoints would silently drop the sources in
between. A reversed or implausibly long range is not a range; those keep their endpoints only.
...
def merge_adjacent(text):
    ...
    def _one(m):
        ...
        return "[" + ", ".join(f"c{i}" for i in ids_) + "]"
...
def sub(text, fn):
    ...
    Both this and merge_adjacent read ids through _block_ids, NOT a bare digit scan: a bare scan sees
    "[c221-c227]" as the two endpoints, so dropping one id would silently delete the five in between.
    """
